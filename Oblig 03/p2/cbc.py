...
def pad(data):
    padding_needed = 2 - (len(data) % 2)
    padding_byte = padding_needed.to_bytes(1,"big")
    return data + padding_byte*padding_needed

#Implementation of unpadding
def unpad(data):
    padding_byte = int.from_bytes(data[-1], 'big')
    return data[:-padding_byte]

def main():
    if len(sys.argv) != 6:
        print("Usage: " + sys.argv[0] + " penc/pdec PLAINTEXT_FILE KEY_FILE IV_FILE OUTPUT_FILE")
    
    else:
        mode = sys.argv[1] # Define which mode
        ...
        # If, else for the two modes

        if mode == "penc":

            padded_data = pad(data) # pad

            plaintext_blocks = splitIntoBlocks(bytes_to_bits(padded_data), 16)
            ciphertext_blocks = cbc_enc(plaintext_blocks, iv, key)
            ciphertext_bytes = bits_to_bytes(mergeBlocks(ciphertext_blocks))
            write_file(output_file, ciphertext_bytes)

        else:
            ciphertext_blocks = splitIntoBlocks(bytes_to_bits(data),16)
            plaintext_blocks = cbc_dec(ciphertext_blocks, iv, key)
            plaintext_bytes = bits_to_bytes(mergeBlocks(plaintext_blocks))

            unpadded_data = unpad(plaintext_bytes) # unpad
